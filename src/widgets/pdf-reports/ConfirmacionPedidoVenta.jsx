...
const Confirmacion_Pedido_Venta = ({
  data, price, title,
  ...
  const cabinets = useMemo(() => {
    if (!data?.cabinets || !Array.isArray(data.cabinets)) return groupCabinetsByType([]);
    const mapped = data.cabinets.map((item) => ({ ...item, total: item.total || 0 }));
    return groupCabinetsByType(mapped);
  }, [data]);
  ...
  return (
    <Document title="Presupuesto COOHOM">
      <Page style={{ paddingHorizontal: "55", paddingVertical: "45" }} size={"A4"} wrap>
        ...
        <CabinetSections cabinets={cabinets} price={price} coeficiente={coeficiente} complementos={complementos} />
...
export default Confirmacion_Pedido_Venta;
